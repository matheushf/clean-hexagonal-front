...
export function useHome() {
  ...
  async function saveFavouriteDog(dog: DogE) {
    try {
      ...
      const response = await DogRepository.home.saveFavouriteDog(dog);
  ...
  async function removeFavoriteDog(favoriteDog: FavoriteDogE) {
    try {
      ...
      await DogRepository.home.removeFavouriteDog(favoriteDog);
    } catch (error) {
      setFavouriteDogs(favouriteDogs);
    }
  }
  ...
  async function fetchFavouriteDogs() {
    const response = await DogRepository.home.getFavouriteDogs();
